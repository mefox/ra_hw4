...
#Homework 3 Robot Autonomy 16-662
#Marshall Fox mefox
...
class RoboHandler:
  ...
  def run_problem_rrt(self):
    ...
    time.sleep(5) # Time for video purposes
...
#    return goal_dofs
  ...
  # The supporting functions are provided right after this function
  #######################################################
  def rrt_to_goal(self, goals):
    print 'Starting the RRT Algorithm'
    #time.sleep(25) #Added for video purposes
    # Initialize the start state
    goals = np.array(goals) # Convert the goals to an np array
    ...
    tree = [] # Initialize the tree that will hold the points Check brackets if does not work
    tree.append(q_initial_tuple) # Add the first element to the tree
    parent = {} # Data structure to store the parents
    ...
    dist, closest_goal, closest_point = self.min_euclid_dist_many_to_many(goals, self.convert_from_dictkey(tree))

    print 'Completed initialization for the RRT, entering loop for building the tree'
